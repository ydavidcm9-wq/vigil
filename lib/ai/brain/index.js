...
async function brainChat(message, options = {}) {
  ...
  // 5. If LLM returned nothing, build answer from KB
  if (!response && promptCtx.kbHits.length) {
    response = 'Here is what I found in the Vigil Knowledge Base:\n';
    for (const entry of promptCtx.kbHits.slice(0, 5)) {
      response += `\n**[${entry.id}] ${entry.title}**\n${entry.content}\n`;
      if (entry.mitigation) response += `\n_Mitigation:_ ${entry.mitigation}\n`;
    }
    if (promptCtx.suggestedActions.length) {
      response += '\n**Suggested next steps:**\n';
      for (const a of promptCtx.suggestedActions.slice(0, 3)) {
        response += `- **Go to: ${a.name}** (${a.targetSection} section)\n`;
      }
    }
  } else if (!response) {
    response = 'No AI provider is configured. Go to **Settings** to set up Claude API or Ollama.\n\nMeanwhile, try asking about specific topics — I can answer from my built-in knowledge base.\nExamples: "T1059", "port 445", "CWE-89", "OWASP Top 10"';
  }

  // 6. Extract profile updates from user message
  const profileUpdates = extractProfileUpdates(message);
  if (profileUpdates) {
    updateProfile(userId, profileUpdates);
  }

  // 7. Extract memories asynchronously (don't block response)
  setImmediate(() => {
    try {
