...
const { execCommand, execFileSafe, binaryExists } = require('./exec');
...
async function runNmap(target, options = {}) {
  ...
  const result = await execFileSafe('nmap', args, { timeout: SCAN_TIMEOUT });
...
async function runNuclei(target, options = {}) {
  ...
  const startTime = Date.now();
  const result = await execFileSafe('nuclei', args, { timeout: SCAN_TIMEOUT });
  const duration = Date.now() - startTime;
...
async function runTrivy(target, type = 'image', options = {}) {
  ...
  const args = [type, '--format', 'json', '--quiet'];
  ...
  const startTime = Date.now();
  const result = await execFileSafe('trivy', args, { timeout: SCAN_TIMEOUT });
  const duration = Date.now() - startTime;
...
async function runSSLCheck(domain, port = 443) {
  ...
  const safePort = Number.isFinite(Number(port)) ? Number(port) : 443;
  if (!safeDomain || safePort < 1 || safePort > 65535) {
    return { available: true, error: 'Invalid SSL target', findings: [] };
  }

  // Get certificate info
  const cmd = `echo | openssl s_client -connect ${safeDomain}:${safePort} -servername ${safeDomain} 2>/dev/null | openssl x509 -noout -text -dates -subject -issuer 2>/dev/null`;
  ...
  if (result.code !== 0 || !result.stdout) {
    findings.push({
      severity: 'critical',
      title: `SSL connection failed for ${safeDomain}:${safePort}`,
      description: 'Could not establish SSL/TLS connection',
      target: `${safeDomain}:${safePort}`,
      cve: null,
  ...
  if (notAfterMatch) {
    ...
    if (certInfo.daysRemaining < 0) {
      findings.push({
        severity: 'critical',
        title: `SSL certificate EXPIRED for ${safeDomain}`,
      ...
      findings.push({
        severity: 'high',
        title: `SSL certificate expiring soon for ${safeDomain}`,
      ...
      findings.push({
        severity: 'medium',
        title: `SSL certificate renewal recommended for ${safeDomain}`,
        description: `Certificate expires in ${certInfo.daysRemaining} days`,
        target: `${safeDomain}:${safePort}`,
        cve: null,
      ...
      findings.push({
        severity: 'info',
        title: `SSL certificate valid for ${safeDomain}`,
  ...
  if (output.includes('sha1WithRSA') || output.includes('md5WithRSA')) {
    findings.push({
      severity: 'high',
      title: `Weak signature algorithm for ${safeDomain}`,
  ...
  if (keyMatch) {
    ...
    if (certInfo.keySize < 2048) {
      findings.push({
        severity: 'high',
      title: `Weak key size (${certInfo.keySize}-bit) for ${safeDomain}`,
